...
use crate::{
    activity::{activity::ActivityDeserializationError, timebox::TimeSlotKind, Activity},
    time::Duration,
    Time,
};
...
impl FromStr for TimeSlotTemplate {
    type Err = TemplateTimeDeserializationError;
...
impl FromStr for TimeTemplate {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let hours_token = time_tokens
            .next()
            .ok_or(TemplateTimeDeserializationError::CantParseTime)?;
        ...
        let hours = if hours_token == "%H" {
            ...
            Err(TemplateTimeDeserializationError::CantParseTime)
        }?;
...
#[derive(Error, Debug)]
pub enum TemplateTimeDeserializationError {
    #[error("can't parse")]
    CantParse,
    #[error("can't parse time")]
    CantParseTime,
}

#[derive(Error, Debug)]
pub enum TemplateDeserializationError {
    #[error("time box deserialization error")]
    Disconnect(#[from] TemplateTimeBoxDeserializationError),
}

impl FromStr for Template {
    type Err = TemplateDeserializationError;
...
#[derive(Error, Debug)]
pub enum TemplateTimeBoxDeserializationError {
    #[error("empty string")]
    EmptyString,
    ...
    InvalidTime(#[from] TemplateTimeDeserializationError),
}

const MARKDOWN_LIST_TOKENS: &[char] = &['-', '*'];

impl FromStr for TimeBoxTemplate {
    type Err = TemplateTimeBoxDeserializationError;
...
impl<'t> TemplateTimeBoxParser<'t> {
    pub fn from_tokens(tokens: Peekable<SplitWhitespace<'t>>) -> Self {
        TemplateTimeBoxParser { tokens }
    }

    pub fn generate(mut self) -> Result<TimeBoxTemplate, TemplateTimeBoxDeserializationError> {
        let first = self.tokens.peek();

        if first == None {
            return Err(TemplateTimeBoxDeserializationError::EmptyString);
        }
        ...
        while let Some(next) = self.tokens.next() {
            match Self::token_kind(next) {
                ...
                TokenKind::Time => {
                    ...
                    time = Some(TimeSlotTemplate::from_str(next)?);
                }
        ...
        match activity {
            ...
            None => {
                return Err(TemplateTimeBoxDeserializationError::NoActivity);
            }
        }
    }
